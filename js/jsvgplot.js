...
function createCoords(svgid,parameters) {
  ...
  this.y = function(v,u) { if( u === undefined ) { u = 0; } return( (v-f)/d ); }
	...
	this.ruv = Math.sqrt( (this.uMax-this.uMin)*(this.uMax-this.uMin)+(this.vMax-this.vMin)*(this.vMax-this.vMin) );
  this.rxy = Math.sqrt( (this.xMax-this.xMin)*(this.xMax-this.xMin)+(this.yMax-this.yMin)*(this.yMax-this.yMin) );
  this.uv = function(xy) { return( xy*this.ruv/this.rxy ); }
	this.xy = function(uv) { return( uv*this.rxy/this.ruv ); }
  this.ru = function(x) { return( x*Math.abs((this.uMax-this.uMin)/(this.xMax-this.xMin)) ); } 
  this.rv = function(y) { return( y*Math.abs((this.vMax-this.vMin)/(this.yMax-this.yMin)) ); }
  this.rx = function(u) { return( u*Math.abs((this.xMax-this.xMin)/(this.uMax-this.uMin)) ); } 
  this.ry = function(v) { return( v*Math.abs((this.yMax-this.yMin)/(this.vMax-this.vMin)) ); }
...
createCoords.prototype.plotAxis = function (axis,attributes,axisSVG,minorSVG,labelSVG) {
  ...
  let axisGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');  
	...
	attributes = Object.assign({'x0':0,'y0':0,'x1':this.xMin,'x2':this.xMax,'y1':this.yMin,'y2':this.yMax},attributes); // coordinate stuff
	attributes = Object.assign({'tick':1,'tickL':this.ry(this.marginBottom/5),'minorN':0,'axisH':0.5,'axisV':0.5,'append':this.svg.g2},attributes);
	attributes['minorL'] = attributes['minorL'] == undefined ? attributes['tickL']*2/3 : attributes['minorL'];
	axisSVG = Object.assign({'stroke':'black','stroke-width':3},axisSVG );
	labelSVG = Object.assign({'font-size':'2rem'},labelSVG);

	let i,d,x0,y0,x1,x2,y1,y2,x,y,dx,dy,t0,xt,yt;
	x0 = attributes['x0']; y0 = attributes['y0'];
	x1 = attributes['x1']; y1 = attributes['y1'];
	x2 = attributes['x2']; y2 = attributes['y2'];
	let n1,n2,n,showX0,showY0,dm;
	...
	if( axis == 'xy' || axis == 'x' ) {
		...
		if( attributes['xpts'] != undefined ) { xpts = attributes['xpts']; }
			...
			if( showX0) { xpts[i] = x0; i++; }
			for( x = x0+dx; x <= x0+n2*dx; x+= dx ) { xpts[i] = x; i++; }
			n = i;		
		}
		n = xpts.length;
		// get X axis labels
		if( attributes['xtxt'] != undefined && attributes['xtxt'].length >= n ) { xtxt = attributes['xtxt']; }
		else { for( i = 0; i < n; i++ ) { xtxt[i] = xpts[i]; } }		
		
		// plot x-axis
		d='';
		yt = attributes['xt'] != undefined ? attributes['xt'] : y0-xtickL;
		let dv = attributes['dv'] != undefined ? attributes['dv'] : 0;
		for( i = 0; i < n; i++ ) { // plot major ticks and labels
			x = xpts[i];
			d += 'M '+x+' '+' '+y0+' v '+(-xtickL)+' ';			
			this.plotObject('path',{'d':d,'append':xAxis},axisSVG);
			this.plotText(xtxt[i],{'x':x,'y':yt,'append':xAxis,'dv':dv},Object.assign({'dominant-baseline':'hanging'},labelSVG)	);
			// add last few minor axis
			for ( let j = 1; j <= minorNx; j++ ) {
				if( x-j*dm >= x1 ) {
					d += 'M '+(x-j*dm)+' '+' '+y0+' v '+(-minorLx)+' ';			
					this.plotObject('path',{'d':d,'append':xAxis},axisSVG);
				}
			}
		}
	...
	if( axis == 'xy' || axis == 'y' ) {
		...
		ytickL2 = ytickL*this.plotBox[3]/this.plotBox[2]; this.ytickL2 = ytickL2;
		minorLy2 = minorLy*this.plotBox[3]/this.plotBox[2];		
		...
		if( attributes['ypts'] != undefined ) { ypts = attributes['ypts']; }
			...
			n = n1+n2+ ( y0 == this.yMin || attributes['showY0'] == true ) ? 1 : 0;
			i = 0; for( y = y0-n1*dy; y < y0; y+= dy ) { ypts[i] = y; i++; }
			if( showY0) { ypts[i] = y0; i++; }
			for( y = y0+dy; y <= y0+n2*dy; y+= dy ) { ypts[i] = y; i++; }
			n = i;		
		}
		n = ypts.length;
		// get Y axis labels
		if( attributes['ytxt'] != undefined && attributes['ytxt'].length >= n ) { ytxt = attributes['ytxt']; }
		else { for( i = 0; i < n; i++ ) { ytxt[i] = ypts[i]; } }		
	
		// plot y-axis
		dy = attributes['tickY'];
		d='';
		xt = attributes['xt'] != undefined ? attributes['xt'] : x0-ytickL2;
		let du = attributes['du'] != undefined ? attributes['du'] : -2;
		for( i = 0; i < n; i++ ) { // plot major ticks and labels
			y = ypts[i];
			d += 'M '+x0+' '+' '+y+' h '+(-ytickL2)+' ';			
			this.plotObject('path',{'d':d,'append':yAxis},axisSVG);
			this.plotText(ytxt[i],{'x':xt,'y':y,'append':yLabels,'du':du},Object.assign({'text-anchor':'end','font-size':'2rem'},labelSVG));
			// add minor axis points
			for ( let j = 1; j <= minorNy; j++ ) {
				if( y-j*dm >= y1 ) {
					d += 'M '+x0+' '+' '+(y-j*dm)+' h '+(-minorLy2)+' ';			
					this.plotObject('path',{'d':d,'append':yAxis},axisSVG);
				}
			}
		}
...
createCoords.prototype.plotAxisText = function (axis,attributes,textSVG) {	
	textSVG = Object.assign({'stroke':'black','stroke-width':1,'fill':'black','font-size':'2rem','dominant-baseline':'middle','text-anchor':'middle','font-family':'Calibri'},textSVG == undefined ? {} : textSVG );

	// defines the space below the tickmark labels and the edge of the graph, expect parameters as a fraction of this space
	let xaxisBox = new Array(); // u,v,uwidth,vheight
	let yaxisBox = new Array();
	xaxisBox = [this.x(this.uMin),this.y(this.vMax+this.fontSize*this.plotBox[2]/this.plotBox[3])-this.xtickL,this.x(this.uMax),this.y(this.svg.clientHeight)];
	yaxisBox = [-this.x(this.marginLeft*2),this.yMin,this.x(this.uMin-this.fontSize-2)-this.ytickL2,this.yMax];
	attributes['axisH'] = attributes['axisH'] == undefined ? 0.5 : attributes['axisH'];
	attributes['axisV'] = attributes['axisV'] == undefined ? 0.5 : attributes['axisV'];
	let append = attributes['append'] != undefined ? attributes['append'] : this.svg.g2;
	
	if( axis == 'x' || axis == 'xy' ) {
		let xaxisText = attributes['xaxisText'] != undefined ? attributes['xaxisText'] : attributes['axisText'];
		let xaxisH = attributes['xaxisH'] != undefined ? attributes['xaxisH'] : attributes['axisH'];
		let xaxisV = attributes['xaxisV'] != undefined ? attributes['xaisiV'] : attributes['axisV'];

		let x = xaxisBox[0]+xaxisH*(xaxisBox[2]-xaxisBox[0]);
		let y = xaxisBox[1]+xaxisV*(xaxisBox[3]-xaxisBox[1]);
	
		let temp = {'x':x,'y':y,'du':(attributes['du'] != undefined ? attributes['du'] : 0),'dv':(attributes['dv'] != undefined ? attributes['dv'] : 0),'append':append}
		if( attributes['rotatex'] != undefined ) { temp = Object.assign({'rotate':attributes['rotatex']},temp); }
		let xaxis = attributes['xaxis'] != undefined  ? attributes['xaxis'] : 'x-axis label';
		this.plotText(xaxis,temp,textSVG);		
	}
	if( axis == 'y' || axis == 'xy' ) { 
		let yaxisText = attributes['yaxisText'] != undefined ? attributes['yaxisText'] : attributes['axisText'];
		let yaxisH = attributes['yaxisH'] != undefined ? attributes['yaxisH'] : attributes['axisH'];
		let yaxisV = attributes['yaxisV'] != undefined ? attributes['xaisiV'] : attributes['axisV'];
		let x = yaxisBox[0]+yaxisH*(yaxisBox[2]-yaxisBox[0]);
		let y = yaxisBox[1]+yaxisV*(yaxisBox[3]-yaxisBox[1]);	
		let yaxis = attributes['yaxis'] != undefined  ? attributes['yaxis'] : 'y-axis label';
		let rotate = attributes['rotatey'] != undefined ? attributes['rotatey'] : ( attributes['rotate'] == undefined ? attributes['rotate'] : '-90' );
		this.plotText(yaxis,{'x':x,'y':y,'append':append,'rotate':rotate,'du':(attributes['du'] != undefined ? attributes['du'] : 0),'dv':(attributes['dv'] != undefined ? attributes['dv'] : 0)},textSVG);	
		this.plotObject('circle',{'cx':x,'cy':y,'append':append},{'r':2,'fill':'red','opacity':0.8});
	}
...
createCoords.prototype.plotText = function(text,attributes,textSVG) { 
  ...
  let y = attributes['y'];
	...
	if( attributes['rotate'] != undefined ) {
		let rotate = 'rotate('+attributes['rotate']+' '+(this.u(x)+du)+' '+(this.v(y)+dv)+')';
	...
	setAttributes(obj,{'x':this.u(x+dx,y)+du,'y':this.v(y+dy,x)+dv});
...
createCoords.prototype.plotObject = function(objType,attributes,objSVG) {
	...
	if ( attributes == undefined ) { var attributes = {}; }	
	let attach = attributes['clip'] != undefined ? document.getElementById(attributes['clip']) : this.svg.g1;	
	
	// SET attributes of the SVG object using COORDS parameters
	  // use
  if( objType == 'use' ) {  // HAVEN'T TESTED THIS NEW VERSION YET   
    var dx = attributes['dx'] !== undefined ? attributes['dx'] : 0;
    var dy = attributes['dy'] !== undefined ? attributes['dy'] : 0;
    var id = attributes['id'];
                                                                
    objSVG['x'] = this.u(attributes['x']+dx,attributes['y']+dy);
    objSVG['y'] = this.v(attributes['y']+dy,attributes['x']+dx);        
    obj.setAttributeNS('http://www.w3.org/1999/xlink','xlink:href','#'+id);
  }
	  // group
  else if( objType == 'g' ) { } // might do something with this later	
	  // circle
  else if( objType == 'circle' && attributes != false ) {
    tempx = attributes['cx'] != undefined ? attributes['cx'] : attributes['x'];
    tempy = attributes['cy'] != undefined ? attributes['cy'] : attributes['y'];  	
    objSVG['cx'] = this.u(tempx,tempy);
    objSVG['cy'] = this.v(tempy,tempx);	
    if( attributes['r'] != undefined ) { if( !re.test(attributes['r']) ) { objSVG['r'] = this.uv(attributes['r']); } }
		if( attributes['stroke-width'] != undefined ) { if( !re.test(attributes['stroke-width']) ) { objSVG['stroke-width'] = this.r(attributes['stroke-width']); } }
  }
	  // line
  else if ( objType == 'line' ) { 
    objSVG['x1'] = this.u(attributes['x1']);
    objSVG['y1'] = this.v(attributes['y1']);
    objSVG['x2'] = this.u(attributes['x2']);
    objSVG['y2'] = this.v(attributes['y2']); 
  } 
	  // rect
  else if ( objType == 'rect' ) {    
    let u1,u2,v1,v2;        
    if( attributes['width'] !== undefined ) {
      u1 = this.u(attributes['x'],attributes['y']);
      u2 = this.u(attributes['x']+attributes['width'],attributes['y']);
      v1 = this.v(attributes['y'],attributes['x']);
      v2 = this.v(attributes['y']+attributes['height'],attributes['x']);    
    } else {
      u1 = this.u(attributes['x1'],attributes['y1']);
      u2 = this.u(attributes['x2'],attributes['y2']);
      v1 = this.v(attributes['y1'],attributes['x1']);
      v2 = this.v(attributes['y2'],attributes['x2']);
      delete attributes['x1']; delete attributes['x2'];
      delete attributes['y1']; delete attributes['y2'];
    }
    if( !re.test(attributes['rx']) ) { objSVG['rx'] = this.rx(attributes['rx']); } 
    if( !re.test(attributes['ry']) ) { objSVG['ry'] = this.ry(attributes['ry']); } 
    ...
    objSVG['d'] = this.parsePath(attributes['d']);                
  }
  else if( objType == 'clip' ) {
...
createCoords.prototype.parsePath = function(d) {
	...
	for( i=0; i<darray.length; i++ ) {
		if( /[MmLlTt]/.test(darray[i]) ) {
			...
			dstring+=cmd+' ';
			dstring+=this.u(x,y).toFixed(3)+' ';
			dstring+=this.v(y,x).toFixed(3)+' ';
			i+=2;
		}
		...
		if( /[SsQq]/.test(darray[i]) ) {
			...
			dstring+=cmd+' ';
			dstring+=this.u(x1,y1).toFixed(3)+' ';
			dstring+=this.v(y1,x1).toFixed(3)+' ';
			dstring+=this.u(x,y).toFixed(3)+' ';
			dstring+=this.v(y,x).toFixed(3)+' ';
			i+=4;
		} 
			...
			dstring+=this.u(x1,y1).toFixed(3)+' ';
			dstring+=this.v(y1,x1).toFixed(3)+' ';
			dstring+=this.u(x2,y2).toFixed(3)+' ';
			dstring+=this.v(y2,x2).toFixed(3)+' ';
			dstring+=this.u(x,y).toFixed(3)+' ';
			dstring+=this.v(y,x).toFixed(3)+' ';
			i+=6;
		}      
			...
			dstring+=cmd+' ';
			dstring+=this.ru(darray[i+1]).toFixed(3)+' '; // rx
			dstring+=this.rv(darray[i+2]).toFixed(3)+' ' // ry
...
createCoords.prototype.plotFunction = function(func,funcParam,pathParam,pathSVG,pointsParam,pointsSVG) {
  ...
  let plotGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');  
	...
	pathParam = Object.assign({'n':100,'z':false,'x1':0,'x2':20}, pathParam == undefined ? {} : pathParam );	
  ...
  if( (pathParam['xpts'] === undefined || pathParam['ypts'] === undefined) ) { 
    let x1 = pathParam['x1'] !== undefined ? pathParam['x1'] : this.xMin;
    let x2 = pathParam['x2'] !== undefined ? pathParam['x2'] : this.xMax;    
	...
	path = ( xpts[0] >= this.xMin ) ? 'M '+xpts[0]+' '+this.y0+' L '+xpts[0]+','+ypts[0] : 'M '+xpts[0]+','+ypts[0];
	for( i=1; i <= n; i++ ) { path+=' L '+xpts[i]+','+ypts[i]; }
	if( xpts[n] <= this.xMax ) { path+=' L '+xpts[n]+' '+this.y0; }
